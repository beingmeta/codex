  ...
  (function(){
    ...
    Codex.addConfig
      ("pageview",
       function(name,val){
	if (val) {
	  // If the init hasn't been processed yet, wait with
	  // paginating, since it may change font sizes, styles, etc.
	  if (Codex.initconfig) repaginate();
	  else Codex.paginate=true;}
	else {
	  clearPagination();
