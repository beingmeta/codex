	...
	function Paginate(callback){
		...
		if (oldpage===newpage) {
			...
			if ((trace)&&(typeof trace === 'number')&&(trace>1))
			    fdjtLog("forceBreak/%D@%o h=%o off=%o tm=%o geom=%j",
				    elt,newpage,height,pageoff,top_margin,g);}
		    else {
			top_margin=height-(g.top-booktop);
			if ((trace)&&(typeof trace === 'number')&&(trace>1))
			    fdjtLog("forceBreak/%D@%o h=%o tm=%o geom=%j",
				    elt,newpage,height,top_margin,g);}
		    ...
		    fdjtLog("forceBreak/%d@%o h=%o geom=%j",
			    elt,newpage,height,g);
		else {}
